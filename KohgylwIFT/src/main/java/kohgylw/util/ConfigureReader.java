...
/**
 * 配置文件解析类
 * <p>该类中封装了对configure文件的解析操作，其实例由instance方法提供</p>
 * 配置格式：
...
public class ConfigureReader {
	...
	/**
	 * 判断用户是否在配置中声明
	 * <p>将传入用户与配置文件中的用户列表进行比对，如果找到该用户，则返回true，否则返回false</p>
	 * @param account String 要查找的账户
	 * */
	...
	/**
	 * 检查账户与密码是否匹配
	 * <p>对传入的账户及密码与配置文件中的信息进行比对，如果匹配则返回true，否则返回false</p>
	 * @param account String 需要检查的账户
	 * @param pwd String 需要检查的密码
	 * */
	...
	/**
	 * 判断用户是否具备某权限
	 * <p>检查配置中用户是否具备对传入的权限</p>
	 * @see AccountAuth
	 * @param account String 需要检查的用户
	 * @param auth AccountAuth 需要检查的权限
	 * */
	public boolean authorized(String account,AccountAuth auth) {
		if(confp!=null) {
			String auths=confp.getProperty("auth."+account);
			switch (auth) {
	...
	/**
	 * 得到用户设置的下载缓存大小
	 * <p>该值越大则能提供的下载速度越快，但是内存占用也越高，如果设置无效或者未设置，则默认为1048576</p>
